    ...
    def _create_dgcf_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            for f in range(self.n_fold): # save memory
                ...
                if k > 0 and args.residual in ['residual']: # cross-hop sum
                    print('using residual module')
                    residual_embed.append(tf.sparse_tensor_dense_matmul(A_fold_hat[f], all_embeddings[k-1]))
            
            ego_embeddings = tf.concat(temp_embed, 0)

            if k > 0 and args.residual in ['residual']:
                residual_embeddings = tf.concat(residual_embed, 0)
                ego_embeddings += residual_embeddings

            # local_embeddings = tf.multiply(self.weights['locality_%d' % k], ego_embeddings)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    """
    plain_adj, norm_adj, norm_adj_no_eye, cross_adj = data_generator.get_adj_mat()
